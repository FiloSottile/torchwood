...
func main() {
	...
	for i := range signers {
		// Use the sole, required Ed25519 signer. If -key flag value
		// was in deprecated hex-encoded format, it has been replaced
...
func connectToSSHAgent() []*signer {
	...
	for _, s := range aSigners {
		...
		case ssh.KeyAlgoED25519:
			...
			if ss.sshFP() != *keyFlag {
				// For backwards compatibility, also accept a
				// hex-encoded SHA-256 hash of the public key, which
				// is what -key used to be.
				hh := sha256.Sum256(ss.Public().(ed25519.PublicKey))
				h := hex.EncodeToString(hh[:])
				if h == *keyFlag {
					slog.Warn("flag -key is in deprecated hex-encoded format", "deprecated", fmt.Sprintf("%0x", hh), "fingerprint", ss.sshFP())
					// Replace user's flag value with the fingerprint
					*keyFlag = ss.sshFP()
				}
			}
			if ss.sshFP() == *keyFlag {
	...
	var foundAlgoFPs []string
	for _, signer := range signers {
		foundAlgoFPs = append(foundAlgoFPs, fmt.Sprintf("%s|%s", signer.s.PublicKey().Type(), signer.sshFP()))
	}
	if *keyFlag != "" {
		algoFP := fmt.Sprintf("%s|%s", ssh.KeyAlgoED25519, *keyFlag)
		if !slices.Contains(foundAlgoFPs, algoFP) {
			fatal("ssh-agent is missing key", "expected", algoFP, "available", strings.Join(availableKeys, " "))
		}
	}
	if *mldsaKeyFlag != "" {
		algoFP := fmt.Sprintf("%s|%s", algoMLDSA44, *mldsaKeyFlag)
		if !slices.Contains(foundAlgoFPs, algoFP) {
			fatal("ssh-agent is missing key", "expected", algoFP, "available", strings.Join(availableKeys, " "))
		}
	}
	slog.Info("found keys", "fingerprints", strings.Join(foundAlgoFPs, " "))
