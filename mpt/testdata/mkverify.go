...
// Mkverify generates testdata/verify.txt, a file of test vectors for [mpt.VerifyLookup].
//
// Usage:
//
...
func (g *gen) verifyPrefix(want bool) {
	g.printf("verifyprefix %v\n", want)
}
...
// newTextFixture is like newFixture but stores the names themselves
// as keys, so that the keys share prefixes.
func newTextFixture(names ...string) *fixture {
	t := mpt.NewMemTree()
	for _, name := range names {
		t.Set(mpt.Key(name), tval(name))
	}
	v := int64(1)
	if len(names) == 0 {
		v = 0
...
func (f *fixture) path(key mpt.Key) mpt.Proof {
	path, err := f.tree.Path(key)
	if err != nil {
		panic(err)
	}
	return path
...
func (f *fixture) provePrefix(prefix []byte) (mpt.Hash, mpt.Proof) {
	hash, proof, err := mpt.ProvePrefix(prefix, f.path(prefix))
	if err != nil {
		panic(err)
	}
	return hash, proof
...
func (f *fixture) prove(key mpt.Key) (mpt.Val, bool, mpt.Proof) {
	...
	v, ok, p, err := mpt.ProveLookup(key, path)
	if err != nil {
		panic(err)
	}
	return v, ok, p
...
func (g *gen) header() {
	g.comment("Test vectors for mpt.VerifyLookup, generated by gen.go.")
	g.comment("")
	...
	g.comment("  verify true         - VerifyLookup should succeed (return nil)")
	g.comment("  verify false        - VerifyLookup should fail (return error)")
	g.comment("  prefix HEXPREFIX    - set the key prefix")
	g.comment("  prefix ''           - set the key prefix to empty (zero-length)")
	g.comment("  hash HEXHASH        - set the claimed subtree hash")
	g.comment("  verifyprefix true   - VerifyPrefix should succeed (return nil)")
	g.comment("  verifyprefix false  - VerifyPrefix should fail (return error)")
	g.comment("")
...
func (g *gen) prefixProofs() {
	g.section("Prefix proofs")

	empty := mpt.EmptyTreeHash()

	// Empty tree.
	e := newFixture()

	g.comment("Empty tree: empty proof proves no keys have the prefix.")
	g.snap(e.snap.Hash)
	g.prefix([]byte("a"))
	g.hash(empty)
	g.rawProof(nil)
	...
	g.comment("Empty tree: a non-empty subtree cannot be claimed.")
	g.snap(e.snap.Hash)
	g.prefix([]byte("a"))
	g.hash(sha256.Sum256([]byte("subtree")))
	g.rawProof(nil)
	g.verifyPrefix(false)
	g.blank()

	// Tree with prefix-sharing text keys.
	f := newTextFixture("a", "ab", "abc", "abd", "b", "ba")

	g.comment("Whole tree: the empty prefix selects the root, with no path steps.")
	g.snap(f.snap.Hash)
	g.prefix(nil)
	g.hash(f.snap.Hash)
	g.rawProof(nil)
	...
	hab, pab := f.provePrefix([]byte("ab"))
	g.comment("Prefix ab: subtree holding ab, abc, and abd.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("ab"))
	g.hash(hab)
	g.proof(pab, true)
	...
	g.comment("Prefix ab: valid proof but wrong subtree hash.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("ab"))
	g.hash(sha256.Sum256([]byte("wrong subtree")))
	g.proof(pab, true)
	g.verifyPrefix(false)
	g.blank()

	g.comment("Prefix ab: valid proof but wrong snapshot.")
	g.snap(sha256.Sum256([]byte("wrong")))
	g.prefix([]byte("ab"))
	g.hash(hab)
	g.proof(pab, true)
	...
	g.comment("Prefix ab: truncated proof.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("ab"))
	g.hash(hab)
	g.rawProof(truncated(pab, len(pab)-1))
	g.verifyPrefix(false)
	g.blank()

	g.comment("Prefix ab: proof with trailing garbage.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("ab"))
	g.hash(hab)
	g.rawProof(appended(pab, 0x00))
	g.verifyPrefix(false)
	g.blank()

	habc, pabc := f.provePrefix([]byte("abc"))
	g.comment("Prefix abc: the single abc leaf.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("abc"))
	g.hash(habc)
	g.proof(pabc, true)
	...
	g.comment("Prefix ab: claiming abc's subtree omits ab and abd, so it must fail.")
	g.comment("Its first path step splits at a bit index inside abc but past ab.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("ab"))
	g.hash(habc)
	g.proof(pabc, true)
	...
	g.comment("Prefix ab: claiming the whole tree succeeds. The proof is one-sided:")
	g.comment("it shows the subtree holds every key with the prefix, not that it")
	g.comment("holds nothing else.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("ab"))
	g.hash(f.snap.Hash)
	g.rawProof(nil)
	...
	ha0, pa0 := f.provePrefix([]byte("a\x00"))
	g.comment("Prefix a-NUL: key a padded with NUL then FFs, so only a matches.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("a\x00"))
	g.hash(ha0)
	g.proof(pa0, true)
	...
	hz, pz := f.provePrefix([]byte("z"))
	g.comment("Prefix z: no keys, proved by the path to the key reached instead.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("z"))
	g.hash(hz)
	g.proof(pz, false)
	...
	g.comment("Prefix z: the same claim needs a proof in a non-empty tree.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("z"))
	g.hash(empty)
	g.rawProof(nil)
	...
	g.comment("Prefix b: claiming no keys, but the path ends at a key with the prefix.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("b"))
	g.hash(empty)
	g.proof(f.path([]byte("b")), false)
	g.verifyPrefix(false)
	g.blank()

	g.comment("Prefix ab: claiming no keys, but ab is in the tree.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("ab"))
	g.hash(empty)
	g.proof(f.path([]byte("ab")), false)
	g.verifyPrefix(false)
	g.blank()

	g.comment("Prefix z: valid proof but wrong snapshot.")
	g.snap(sha256.Sum256([]byte("wrong")))
	g.prefix([]byte("z"))
	g.hash(hz)
	g.proof(pz, false)
	...
	g.comment("Prefix z: valid proof with a corrupted sibling hash.")
	g.snap(f.snap.Hash)
	g.prefix([]byte("z"))
	g.hash(hz)
	g.rawProof(flipped(pz, len(pz)-1))
	g.verifyPrefix(false)
	g.blank()
}
